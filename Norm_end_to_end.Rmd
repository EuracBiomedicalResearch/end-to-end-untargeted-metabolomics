...
```{r unsupervised checks, include=FALSE}
...
legend("topleft", inset = c(0, -0.24), col = col_source,
       legend = names(col_source), lty=1, lwd = 2, xpd = TRUE, ncol = 3,
       cex = 0.7, title = "Phenotype", title.cex = 0.8)
plot_pca(pca_res, pc_x = 3, pc_y = 4, col = col_source)
```
...
boxplot(log2(assay(res, "raw")), xaxt = "n", ylab = "log2 abundance",
        col = col_source, outline=FALSE, medlty = "blank", border = col_source,
        boxwex = 0.99 )
...
```{r internal-standard-table, include=FALSE}
...
ompute the median for each sample, followed by determining the median of these
...
- run MRM for top 500 features per samples 
...
#top_500_fts <- apply(tmp_500, MARGIN = 2, function(indices) 
#assay(res, "raw")[indices, ])
#' not really sure why this does not iterates properly over columns
...
#### RLA 
```{r rla-plots evaluate norm method, include=FALSE, fig.cap = "RLA plot for the raw data and different sample normalization method. Note: outliers are not drawn.", fig.width = 5, fig.height = 14}
par(mfrow = c(7, 1), mar = c(0.2, 4.5, 3, 3))

boxplot(rowRla(assay(res, "raw"), group = sampleData(data)$phenotype),
        cex = 0.5, pch = 16, col = col_source, ylab = "RLA",
        border = paste0(col_source, 40), notch = TRUE, cex.main = 1,
        outline = FALSE, xaxt = "n", main = "Raw data", boxwex = 1)
grid(nx = NA, ny = NULL)
legend("topleft", inset = c(0, -0.2), col = col_source,
...
boxplot(rowRla(assay(res, "median_filled_500"),
               group = sampleData(data)$phenotype),
        cex = 0.5, pch = 16, 
        col = col_source, ylab = "RLA", 
        border = paste0(col_source, 40), notch = TRUE, boxwex = 1, 
...
grid(nx = NA, ny = NULL)
abline(h = 0, lty=3, lwd = 1, col = "black")
legend("bottomleft", inset = c(0, -0.2), col = col_source,
       legend = names(col_source), lty=1, lwd = 2, xpd = TRUE, ncol = 3,
       cex = 0.7)
```
...
```{r fit-model-table, message=FALSE, include=FALSE, results=}
...
```{r features-large-slopes-plots, message=FALSE, warning=FALSE, include=FALSE}
...
```{r rla-plot after norm, include = FALSE, fig.cap = "RLA plot before and after normalization. Note: outliers are not drawn."}
