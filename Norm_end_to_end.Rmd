...
In this analysis, all metabolites are considered. We assume that most missing
values (i.e. those not removed because of technical problems) are representing
signal which is below detection limit. In such cases,
...
## Also checked that Imputed value did not affect the PCA: it did not.
```
...
```{r counts, fig.height=7, fig.width=5, include=TRUE}
...
boxplot(log2(assay(res, "raw_filled")), xaxt = "n", ylab = "log2 abundance filled data",
...
We can also observe that the samples are not showing very important variation
in their mean and median. 

RLA

One way to evaluation the distribution of value within our data are relative log
...
- explain Match code and arguments 
- 
...
## - Pretty good mix no ? 
## - PC1 and 2 show again show variance due to QC but definitely not as clear
## as all features PCA
...
#' QC (serum) and study (plasma).- i will write that below
...
RUV only on study samples
```{r test ruv only study, fig.height= 7, fig.width=5}
#' if you agree with the code I will swap it with the previous one
tmp <- assay(res, "raw")[ , res$phenotype != "QC"]

fts_int_std_mean <- colMeans(tmp[intern_standard$feature_id,],
                             na.rm = TRUE)

cor_int_std <- apply(log2(tmp),
                     MARGIN = 1, cor, y = log2(fts_int_std_mean))

neg_ctrls <- which(cor_int_std > 0.6)
neg_ctrls_int <- which(rownames(res) %in% intern_standard$feature_id)

# I think there is no other choice than run on all and then substitute the QC value back to the original ones 
library(MetNorm)
...
ruv_rand <- NormalizeRUVRand(t(log2(assay(res, "raw_filled_imputed"))),
                             ...
                             plotk = FALSE)
fv_ruvr <- 2^t(ruv_rand$newY)
assays(res)$ruvr_study <- fv_ruvr
assay(res, "ruvr_study")[ , res$phenotype == "QC"] <- 
    assay(res, "raw_filled_imputed")[ , res$phenotype == "QC"] 
...
ruv_3 <- RUVIII(t(log2(assay(res, "raw_filled_imputed"))),
                ...
                M = design.matrix(res$sample_name))
fv_ruv3 <- 2^t(ruv_3)
assays(res)$ruv3_study <- fv_ruv3
assay(res, "ruv3_study")[ , res$phenotype == "QC"] <- 
    assay(res, "raw_filled_imputed")[ , res$phenotype == "QC"] 
```
...
vioplot(log2(assay(res, "ruvr")),
        cex = 0.5, col = col_sample, ylab = expression(log[2]~abundance),
        border = paste0(col_sample, 40), xaxt = "n", main = "ruvr", cex.main = 1)
grid(nx = NA, ny = NULL)
abline(h = 0, lty=3, lwd = 1, col = "black")

vioplot(log2(assay(res, "ruvr_study")),
        cex = 0.5, col = col_sample, ylab = expression(log[2]~abundance),
        border = paste0(col_sample, 40), xaxt = "n",
        main = "ruvr study sample only", cex.main = 1)
grid(nx = NA, ny = NULL)
abline(h = 0, lty=3, lwd = 1, col = "black")

vioplot(log2(assay(res, "ruv3")),
        cex = 0.5, col = col_sample, ylab = expression(log[2]~abundance),
        border = paste0(col_sample, 40), xaxt = "n", main = "ruv III", cex.main = 1)
grid(nx = NA, ny = NULL)
abline(h = 0, lty=3, lwd = 1, col = "black")

vioplot(log2(assay(res, "ruv3_study")),
        cex = 0.5, col = col_sample, ylab = expression(log[2]~abundance),
        border = paste0(col_sample, 40), xaxt = "n",
        main = "ruvIII study sample only", cex.main = 1)
grid(nx = NA, ny = NULL)
abline(h = 0, lty=3, lwd = 1, col = "black")


## No changes for ruv3 
## and of course ruvrandom is super dramatic. I would keep the previous.
## Or maybe my strategy was not good. 
```
...
study_sample <- cbind(
    CV_Raw = rowRsd(assay(res, "raw_filled")[, index_study], na.rm = TRUE),
    CV_MRM = rowRsd(
...
```{r}
#' Number <0.3 for both QC and 
Summary <- matrix(ncol= ncol(QC), nrow = 4, dimnames = list(c("CV <0.3 in QC",
                                                  "Mean of CV in QC",
                                                  "CV <0.3 in study samples",
                                                  "Mean of CV in Sutdy samples"
                                                  )))
colnames(Summary) <- colnames(QC)
for (i in seq_len(ncol(QC))){
    Summary["CV <0.3 in QC",i] <- sum(QC[, i] < 0.3, na.rm = TRUE)
    Summary["Mean of CV in QC",i] <- mean(QC[, i], na.rm = TRUE)
}

#' Number <0.3 study samples
for (i in seq_len(ncol(study_sample))){
    Summary["CV <0.3 in study samples",i] <- sum(study_sample[, i] < 0.3, na.rm = TRUE)
    Summary["Mean of CV in Sutdy samples",i] <- mean(QC[, i], na.rm = TRUE)
}

Summary
## remove significant numbers ?
```
...
(phili: i personally find them harder to read in this case..)
...
#' we will just keep the median scaling at the end so:
#' also need to remove 
tmp <- assay(res, "median_filled")
...
#' we can discusss nest week but ye I would rerun everything based on median filled (not 500)
...
#prep df
tmp <- assay(res, "median_filled_500")
tmp[is.na(assay(res, "raw"))] <- NA #' why did you do that, otherwise remove, but remove better 

y <- res$injection_index

# pearson correlation between concentration ??? you mean feature abundance and 
cor(x, y, method = "spearman") 

apply(tmp, MARGIN = 1, function(z) ...cor) #'then sort it and plot and od it for after 
```
...
pandoc.table(intern_standard[, 
             c("mean_abd", "mean_abd_norm", "sd_abd", "sd_abd_norm", "CV_raw", "CV_norm")],
             style = "rmarkdown", caption = cpt)
```
### Sample groups
...
```{r}
#' Number of coefficient variation <0.3 for both QC and study samples
Summary <- matrix(ncol= ncol(sample_res), nrow = 2, 
                  dimnames = list(c("CV <0.3", "Mean of CV")))
colnames(Summary) <- colnames(sample_res)
for (i in seq_len(ncol(sample_res))){
    Summary["CV <0.3",i] <- sum(sample_res[, i] < 0.3, na.rm = TRUE)
    Summary["Mean of CV",i] <- mean(sample_res[, i], na.rm = TRUE)
}

cpt <- paste0("Amount of coefficient of variation below 0.3 for QC and study ",
              "samples, before and after normalization.")
pandoc.table(Summary, style = "rmarkdown", caption = cpt)
```
