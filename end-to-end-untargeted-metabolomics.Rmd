...
Despite the potential insights offered by untargeted LC-MS/MS data, a significant
challenge in the field lies in the absence of a reproducible and scalable
...
gap, we present an innovative R workflow that leverages *XCMS*, and packages of
the *RforMassSpectrometry* environment to encompass all aspects of pre-processing and downstream
...
metabolites and their relative abundance in biological samples. The high
...
and the presence of noise and artifacts. Moreover, the different analysis
steps are generally taking care by specialized packages, which are not always 
easy to integrate in a single workflow. Demonstration the need for a workflow 
with seamless integration of all steps. We therefore propose a comprehensive
workflow for the analysis of LC-MS/MS data using the R environment. We provide 
a toolset to build *customized* workflows with possibility to integrate the
full power of the R universe, compared to (rather limited) graphical user
interface tools. Such workflow are also crucial to the field as they ensure 
reproducibility of the analysis.

Thus, we will go through a highly-detailed and adaptable workflow for LC-MS/MS data
...
possible. The pipeline also includes differential abundance analysis to
identify significant features between groups. The identification of these
features is performed using *MetaboAnnotation* (in conjunction with *SIRIUS*,
put that in annexe),
providing a high level of confidence in the annotation. The workflow is
designed to be highly adaptable to different study designs and analysis
requirements, ensuring reproducibility and scalability.

# Data description

This dataset focuses on investigating the presence of metabolites with
significant differences in the plasma of individuals with a Cardiovascular
disease (CVD) compared to healthy controls (CTRL). The subset comprises
10 mzML files, with three samples each from CVD patients and healthy donors.
...
and *Spectra* packages that integrate multiple Bioconductor packages for
LC-MS/MS analysis. The pre-processing steps are performed using the *xcms*
packages and consist of chromatographic peak detection, retention time alignment
and correspondence/feature definitions. The pre-processing of the data is then followed by
normalization, differential abundance anysis and features annotation using the
*MetaboAnnotation* package with different level of confidence. Lastly, 
*RColorBrewer*, *pander*, *ggplot* and *pheatmap* were used for data 
visualization throughout the pipeline.
...
We next need to set up parallel processing to speed up the analysis. This can be
done using the *BiocParallel* package, which provides a unified interface for 
parallel processing in R. Here, we use the *MulticoreParam* and *SnowParam*
functions to set up parallel processing with 2 cores. The code below checks the
operating system and registers the parallel processing backend accordingly.
This is essential for speeding up the analysis, especially when working with 
large datasets.
...
The experimental data is now a `MsExperiment` object a container for the 
metadata and spectral data. This object type is a core object in the
*RforMassSpectrometry* package, designed to manage the linkage between samples
and spectra. 
...
`sampleData()` function provides access to the phenodata, which contains
information about the sample. Table 1 below build using the *pander* package
...
- Sample Group:Names representing the group in which each of the samples is, 
  the different groups are the followings:
  - QC: Quality control sample
  - CVD: Sample from patient with Cardiovascular disease
  - CTR: Samples from control patients
- Sample Name: unique identifiers for each sample. 
...
time and samples). Below we access the `Spectra` object and check the number of
...
We can also investigate how many spectra and their respective mslevel in each
samples :
...
As explained above the samples were first measured in MS1 mode. The MS2 mode was
only used for a subset of the most intense ions in the MS1 spectra. This second
MS2 run will be introduce later down the pipeline in the annotation process to
identify the significant features between the CVD and CTR groups.
...
We, therefore, have a dataset of `r length(data)` samples, totaling 
`r length(spectra(data))` spectra. 
...
We therefore have `r nrows(featureDefinitions(data))` features defined in our
...
res_ms2 <- readMsExperiment(paste0(path, filename))
