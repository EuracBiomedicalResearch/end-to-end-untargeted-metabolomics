...
res <- filterFeatures(res, filter = filter_rsd)

filter_dratio <- DratioFilter(threshold = 0.5,
                 qcIndex = res$phenotype == "QC",
                 studyIndex = res$phenotype != "QC")
res <- filterFeatures(res, filter = filter_dratio)
```

We could now also remove features based on their detection rate within QC or
study samples. Here due to our very small sample size, we will perform this by
phenotype and will only keep features that are detected into 2/3 of the samples.

```{r eval=FALSE, include=FALSE}
missing_data_filter <- PercentMissingFilter(threshold = 30,
                                       f = res$phenotype)
res <- filterFeatures(res, filter = missing_data_filter)
```
...
Now our data set has been preprocessed, normalized and filtered we can now start to understand the distribution of the data and estimate the variation due to biology.
...
design <- model.matrix(~ 0 + phenotype + age)
fit <- lmFit(log2(assay(res, "norm_imputed")), design = design)

#' Fit the actual contrasts of interest (she made a contrast matrix i'm confused by that)
contr_mat <- makeContrasts(
    CVDvsCTR = phenotypeCTR - phenotypeCVD,
    levels = design)
...
```{r}
tmp <- data.frame(
    coef = fit$coefficient[, "CVDvsCTR"],
    pvalue = fit$p.value[, "CVDvsCTR"],
    adjp = p.adjust(fit$p.value[, "CVDvsCTR"], method = "BH"),
    avg.CVD = rowMeans(
...
Only one feature significant... should prob check before filtering...
...
par(mfrow = c(1, 2))
hist(rowData(res)$pvalue, breaks = 64, xlab = "p value",
     main = "Distribution of raw p-values",
     cex.main = 1, cex.lab = 1, cex.axis = 1)
hist(rowData(res)$adjp, breaks = 64, xlab = expression(p[BH]~value),
...
plot(rowData(res)$coef, -log10(rowData(res)$adjp),
...
rect(xleft = m.cut, ybottom = -log10(p.cut), xright = 100, ytop = 100,
     border = NA, col = paste0(brewer.pal(3, "Set1")[2], 10))
if (any(rowData(res)$significant)) {
    points(rowData(res)$coef[rowData(res)$significant],
           -log10(rowData(res)$adjp[rowData(res)$significant]),
           col = "#0000ffcc")
}
```
...
sum(rowData(res)$significant)
```


```{r result-table, echo = FALSE, results = "asis"}
## Write result table
tab <- rowData(res)[rowData(res)$significant,
