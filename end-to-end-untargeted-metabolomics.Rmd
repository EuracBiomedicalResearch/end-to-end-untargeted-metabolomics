...
evaluate the number of MS1 and MS2 spectra for each of the files.
...
assign a new spectra variable with the employed collision energy.
...
highest intensity for that spectrum.
...
We next remove all mass peaks with a *m/z* value greater or equal to the
precursor *m/z* of the ion and finally restrict to spectra with at least two
fragment peaks.
...
We next extract all fragment spectra from the MassBank database, restrict to
positive polarity data (since also our experiment was acquired in positive
polarity) and perform the same processing to the fragment spectra from the
MassBank database.
...
LLLLLLL maybe first subset to spectra of features and then extract MassBank?
...
Also, evaluating the isotope pattern of a full MS1 scan for a feature can
provide information on element composition () and hence point to the likely
chemical formula for a feature.
