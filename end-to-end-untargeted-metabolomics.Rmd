...
Metabolomics provides a real-time view of the metabolic state of the examined samples. 
Mass spectrometry use in context of metabolomics studies allows to understand the 
#difference in metabolome due to specificc factors. Untargeted LC-MS/MS
As an example, the dataset described here aimed to identify differences in 
plasma metabolite levels between ACM patients and healthy controls.  However one of the challenges in this field is the lack of efficient and scalable infrastructure for the analysis of untargeted MS/MS data.
Currently, numerous specialized packages cater to specific analysis steps but 
often lack flawless integration. 

Metabolomics provides a dynamic snapshot of the metabolic status within examined
samples, with mass spectrometry serving as a key tool in unraveling the 
intricate differences in metabolomes attributable to specific factors. In the 
context of metabolomic investigations, untargeted LC-MS/MS emerges as a powerful
approach. In this study, we employed this technique to discern variations in 
plasma metabolite levels between patients with ACM (Adverse Cardiac Events) and 
healthy controls.
...
gap, we present an innovative R pipeline that harnesses *Spectra* and 
*MsExperiment* to encompass all aspects of pre-processing and downstream 
analyses for LC-MS/MS datasets in a reproducible manner.Our pipeline seamlessly
...
Throughout the entire process it is important to be able to refer to know part 
of the datat set, such as well known ions. In this workflow we selected ions that 
...
The first pre-processing step is to detect the presence of peaks along 
...
param <- CentWaveParam(peakwidth = c(1, 8), ppm = 50, integrate = 2)
...
The EICs confirm the Chromatographic peak detection was successful. 
...
data[1] |>
  ...
  plot() # discuss this, because ugly small peak
```
...
mz_rt <- data.frame(row.names=c("CP114011", "CP113979", "CP114968"), 
                    rtmin = c(155.2120, 181.71800,  12.9190),
                    rtmax = c(201.0710, 228.13500,  56.5460), 
                    mzmin = c(191.0288,  53.50964, 462.1944),
                    mzmax = c(191.0527, 53.53183, 462.3094)) |> 
  as.matrix()
...
param <- MergeNeighboringPeaksParam(expandRt = 2.5, expandMz = 0.001,
                                   minProp = 3/4) 
...
param <- PeakGroupsParam(minFraction = 0.9, extraPeaks = 50, span = 0.3,
...
old_eic_cystine <- chromatogram(data_raw, mz = cystine_mz + c(-0.05, 0.05), 
                                rt = c(205, 214))
plot(old_eic_cystine, main = "Cystine before alignment")

eic_cystine <- chromatogram(data,  mz = cystine_mz + c(-0.05, 0.05), 
                            rt = c(205, 214)) 
plot(eic_cystine, "Cystine after alignment")
```
...
plot(eic_Met, main = "1- Methylhistidine after alignment" )
```
...
#'Updating parameters
param <- PeakDensityParam(sampleGroups = sampleData(data)$phenotype,
                        minFraction = 1/3, binSize = 0.015, bw = 2)
...
Test by checking an area with ions that are isomers: 1-Methylhistidine and 
3-Methylhistidine
```{r}
...
extract results of pre-processing: 
```{r}
...
```
...Describe results
this summarized experiemtn can be subseted by row or column easily: 
...
Te features values are stored as an *assay* wihtin the obejct adn can be accessed using the `assay()` function:
