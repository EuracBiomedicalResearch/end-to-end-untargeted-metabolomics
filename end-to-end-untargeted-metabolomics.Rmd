...
Cardiomyopathy  compared to healthy controls (CTRL). The subset comprises
...
```{r bps, echo=TRUE, fig.width = 12, fig.height = 8, fig.cap = "Base peak spectra for the 10 samples of the experiment.", eval = FALSE}
...
```{r compare-spectra, echo=TRUE, eval=FALSE}
...
```{r Plot bpc before filtering, echo=FALSE, eval=FALSE}
...
```{r EIC extract for internal standard1, include=FALSE}
#' get the list
...
the L-Cystine ion. The labeled methionine internal standard, on the other hand
shows a nice signal but some of noise and a bigger retention time shift between
samples.
...
#' Same for Met
par(mfrow = c(1, 2), mar = c(4, 4.5, 2, 1))
old_eic_met <- chromatogram(data_raw, mz = met_mz + c(-0.005, 0.005),
                            rt =  c(150, 172))
...
jo: would it make sense to discuss the difference with and without `ppm`? Maybe
that's too much. But we should discuss that `ppm` is by default 0, but should be
set to a value > 0 for e.g. TOF instruments.
...
#' 3-Methylhistidine
chr_test <- chromatogram(data, mz = met_mz + c(-0.005, 0.005),
...
F00003 and F00006 contain missing values in some samples
...
#' phili: shouldn't we use raw_filled_imputed here ?
...
plot_pca(pca_res, pc_x = 1, pc_y = 2, pch = 21,
         col = col_sample, bg = paste0(col_sample, 80),
         labels = res$injection_index)
legend("topleft", col = col_phenotype, legend = names(col_phenotype), lty=1,
       lwd = 2,
...
intern_standard$CV_raw <- rowRsd(tmp_fv, na.rm = TRUE, mad = TRUE)
intern_standard$CV_norm <- rowRsd(tmp_fv_norm, na.rm = TRUE, mad = TRUE)
...
I updated the rowRsd() to include MAD. it decreased the CV for quite a lot.
...
performing any statistical analysis. In general, pre-filtering of samples and features is performed to remove outliers.

### Feature prefiltering

Here we will remove the the features that have
QC samples are usually the best to "clean" our dataset or systematic and
random errors. In our dataset, because our QC pool samples are NOT pools of
the samples but pool of other biological samples, we are more limited in their
use for filtering. A in-depth review of the guidelines for data filtering of
untargeted metabolomic studies can be found here:...

We will however still base our pre-filtering on the QC sample of the base that
they are similar bio-fluids of our study samples and therefore relatively
similar metabolites are expected to be seen.
Considering our QC samples, we will first use the previously calculated
coefficient of variation and filter out all features with a coefficient above
30% in these.
...
```{r}
#' Restrict to features that have at least two detected peaks in at least
#' one of the **study** sample groups.
f <- res$phenotype
f[f == "QC"] <- NA
f <- as.factor(f)
...
We next evaluate the RSD (CV) and Dratio for all features in the data set. We
compute these values on the same values that would be used later for the
differential abundance analysis, i.e. the normalized (gap filled and imputed)
feature abundances (assay `"norm_imputed"`).

```{r}
#' Calculate the RSD/CV on the QC samples
rsd <- rowRsd(assay(res, "norm_imputed")[, res$phenotype == "QC"])

#' Calculate the Dratio
dratio <- rowDratio(assay(res, "norm_imputed")[, res$phenotype != "QC"],
                    assay(res, "norm_imputed")[, res$phenotype == "QC"])
```

Below we evaluate the distribution of the RSD and the Dratio and directly
compare the two estimates per feature.

```{r, fig.cap = "Distribution of RSD and Dratio values and comparison of RSD vs Dratio."}
#' jo: puzzling, why are the Dratios so large? Maybe we should skip
#' this comparison?
par(mfrow = c(1, 3))
plot(density(rsd), xlab = "RSD")
grid()
abline(v = 0.3, col = "#ff000080", lty = 3)
plot(density(dratio), xlab = "Dratio")
grid()
plot(rsd, dratio, xlab = "RSD", ylab = "Dratio", pch = 21, col = "#00000060")
grid()
```

```{r}
#' jo: should we consider both filters, or just apply one of the two?
#' would suggest to run only one of the two, and prefer the Dratio over the RSD
#' did also make a more stringend Dratiofilter, with this we get 4 significant
#' features, including caffeine.
## filter_rsd <- RsdFilter(threshold = 0.3, qcIndex = res$phenotype == "QC")
## res <- filterFeatures(res, filter = filter_rsd, assay = "norm_imputed")
...
```
The dataset has been reduced from `r nrow(res)` to `r nrow(res)` features:

Now let's separate the data of QC samples and study samples.
...
col_age <- brewer.pal(6, name = "Blues")
names(col_age) <- res$age
```

First let's observe how our data separate after our adjustment to prevent
variances other than biological.
...
plot_pca(pca_res, pc_x = 1, pc_y = 2, pch = 21,
         col = col_sample, bg = paste0(col_sample, 80),
         labels = res$injection_index)
title(main = "PCA with filtering: sample type", cex = 0.5)
plot_pca(pca_res, pc_x = 3, pc_y = 4, pch = 21,
         col = col_sample, bg = paste0(col_sample, 80),
         labels = res$injection_index)
legend("topright", col = col_phenotype, legend = names(col_phenotype), lty=1,
       lwd = 2, xpd = TRUE, inset = c(0, -0.13), ncol = 3, cex = 0.7,
       title = "Phenotype", title.cex = 0.8,
       bty = "n")
...
plot_pca(pca_res, pc_x = 1, pc_y = 2, pch = 21,
         col = col_age,
         labels = res$injection_index)
title(main = "PCA with filtering: age", cex = 0.5)
plot_pca(pca_res, pc_x = 3, pc_y = 4, pch = 21,
         col = col_age[cut(res$age, length(col_age))],
         labels = res$injection_index)
legend("topright", col = col_age, legend = levels(cut(res$age, length(col_age))),
       cex = 0.7, fill = col_age,
       title = "Age", title.cex = 0.8,
       bty = "n")
...
Linear models
- set the model, one model per metabolites
Is multiple linear regression, but should be only applied to one metabolite by
one as they are heavily correlated to eachother.

In the model we will only include age as it is the only variable between our different samples
...
Generate a data frame with the coefficients, the raw and adjusted p-values
(we apply a Benjamini-Hochberg correction for better
control of the false discovery rate), the average intensity of signals in ARVD
and CTRL samples and whether or not a feature is to be considered significant.
...
- check distribution of raw and adjusted p values
...
#' this is the only code i#m not sure about, i need to check if i can do it another way
...
```{r}
#' just checking the rsd for these features:
rowRsd(assay(res_qc, "norm_imputed")[rownames(tab), ])
```
