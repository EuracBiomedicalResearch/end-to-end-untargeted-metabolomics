...
sim_matrix <- compareSpectra(bps_bin)
pheatmap(sim_matrix)
```
...
```{r}
#' Extract cystine and Methionine m/z for future use.
cystine_mz <- intern_standard["cystine_13C_15N", "mz"]
...
eic_met <- chromatogram(data, rt = c(151, 171), 
...
plot(eic_met, main = "Methionine", col = paste0(col_sample, 80))
grid()
...
... describe EICs 
- Cystine there is a clear concentration difference between
QCs and study samples.
- Methionine: nice signal, bit of noise and rt shift between samples.
...
#' Plot versus spiked 
par(mfrow = c(1, 2))
chromatogram(data, mz = cys_endo + c(-0.01, 0.01), rt = c(200, 220),
             aggregationFun = "max") |>
    plot(main = "Endogenous Cystine", col = paste0(col_sample, 80))
    grid()

plot(eic_cystine, main = "Spiked Cystine", col = paste0(col_sample, 80))
grid()
legend("topright", col = col_phenotype, legend = names(col_phenotype), lty = 1)
```
...
#' Extract and plot EIC of selected internal standard. Remove at some point
eics <- chromatogram(
    data, mz = as.matrix(intern_standard[, c("mzmin", "mzmax")]),
...
for (i in seq_len(nrow(intern_standard))) {
    png(paste0(dr, "EIC_", intern_standard$abbreviation[i], ".png"),
        width = 12, height = 8, units = "cm", pointsize = 4, res = 600)
    plot(eics[i], main = intern_standard$name[i], col = paste0(col_sample, 80))
    grid()
    legend("topright", col = col_phenotype,
           legend = names(col_phenotype), lty = 1)
...
```{r}
#' With more accurate parameters; snthresh reduced because peak detection
#' on an EIC does not have enough data to properly estimate "noise"
...
par(mfrow = c(1, 2))
plot(cystine_test, main = "Cystine", col = paste0(col_sample, 80),
     peakBg = paste0(col_sample, 40)[chromPeaks(cystine_test)[, "column"]])
grid()

plot(met_test, main = "Methionine", col = paste0(col_sample, 80),
...
eic_cystine <- chromatogram(data, mz = cystine_mz + c(-0.01, 0.01),
                            rt = c(205, 215))

eic_met <- chromatogram(data, mz = met_mz + c(-0.01, 0.01),
                        rt = c(155, 165))
```
...
#' Plot
par(mfrow = c(1, 2))
plot(eic_cystine, main = "EIC Cystine", col = paste0(col_sample, 80),
     peakBg = paste0(col_sample[chromPeaks(eic_cystine)[, "sample"]], 40))
grid()

plot(eic_met, main = "EIC Methionine", col = paste0(col_sample, 80),
...
it defines the smoothness of the curve to determine a peak, the default for
...
dataset. We highly recommend using this if in the data set repetitively measured
QC samples (e.g. sample pools) are available. Note also that for subset-based
alignment the samples should be ordered by injection index (i.e. in the order in
...
1-MethylMethionine Extracted Ion Chromatogram (EIC), we need to reduce this
...
```{r}
#' Extract chromatogram with signal for isomers 1-MethylMethionine and
#' 3-MethylMethionine
...
par(mfrow = c(1, 2))
    plot(is_features[1], col = col_sample,
         peakBg = paste0(col_sample[chromPeaks(is_features)[, "sample"]], 40),
         main = "methionine_13C_15N")
    abline(v = intern_standard["methionine_13C_15N", "RT"], lty = 2)
    plot(is_features[2], col = col_sample,
         peakBg = paste0(col_sample[chromPeaks(is_features)[, "sample"]], 40),
         main = "cystine_13C_15N")
...
#' Also create an assay normalised with no filled or imputed data
tmp <- assay(res, "norm")
...
```{r rla-plot after norm, include = TRUE, fig.cap = "RLA plot before and after normalization. Note: outliers are not drawn.", fig.height= 7, fig.width=5.5}
...
No much difference but still the RLA plot showed that it allowed to centered the data around the median...
for bigger and more complex datatset... 
