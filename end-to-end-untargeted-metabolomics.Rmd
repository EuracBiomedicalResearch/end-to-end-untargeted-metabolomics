...
param <- CentWaveParam()
default <- findChromPeaks(eic_cystine, param = param)
chromPeaks(default)

#' Evaluate for 1- Methylhistidine
default <- findChromPeaks(eic_Met, param = param)
chromPeaks(default)
...
Test by checking an area with ions that are isomers of isoleucine and leucine 
```{r}
#' Extract chromatogram with signal for isomers leucine and isoleucine
chr_test <- chromatogram(data, mz = 132.10191 + c(-0.01, 0.01),  
                         rt = c(10, 180),aggregationFun = "max")
...
Process to try to rescue some peaks that are only detected in a subset of samples. 
```{r peaks found in a few QC not all, echo=FALSE}
is.na(featureValues(data))
#' Extract the m/z-rt region for selected peaks
mz_rt <- featureArea(data, features = c("FT00057", "FT00086"))
mz_rt[, "rtmin"] <- mz_rt[, "rtmin"] - 2
mz_rt[, "rtmax"] <- mz_rt[, "rtmax"] + 2

#' Extract their EICs and plot them
chrs <- chromatogram(data[c(1,3)], mz = mz_rt[, c("mzmin", "mzmax")],
                     rt = mz_rt[, c("rtmin", "rtmax")])
plot(chrs, col = c("red", "blue"), lwd = 2)


```
(find better example of peaks that can be rescued): is.na(featureValues(data))
.. Discuss example 
...
sum(is.na(featureValues(data)))
```
With `fillChromPeaks` we could thus rescue signal for all but `r sum(is.na(featureValues(data)))` features. Below we extract the EICs for these and visually inspect some of them.
...
chrs_na <- chromatogram(data[1], mz = mz_rt[, c("mzmin", "mzmax")],
                        rt = mz_rt[, c("rtmin", "rtmax")])
