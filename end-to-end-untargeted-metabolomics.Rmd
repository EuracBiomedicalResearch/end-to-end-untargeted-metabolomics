---
title: "End-to-end workflow for LC-MS/MS analysis using *RforMassSpectrometry* and
  *xcms*"
author:
...
thanks to its versatility and resolution. This paper focuses on a dataset aimed 
at identifying differences in plasma metabolite levels between individuals 
suffering from a cardiovascular disease and healthy controls.
...
to generate data-set specific workflows. Our pipeline seamlessly integrates 
...
a large number of metabolites and their relative abundance in biological
samples. The high sensitivity and specificity of LC-MS/MS make it an 
...
Moreover, due to different instrumental setups and used protocols the
definition of a single *one-fits-all* workflow is impossible. Finally, while
specialized software packages exist for each individual step during an
analysis, their seamless integration remains elusive.
...
Note that not different equipments will generate different files extensions, so a conversion step might be needed before, although it doesnt apply to this dataset. Several tools can be used to convert files to .mzML format, such as ProteoWizard`s MSConvert
...
base R `table()` function and combine the result into a matrix. Note that this is simply the number of spectra acquired at each run, and not the number of spectral features in each sample, thus, they have all the same values.
...
[Dan: Does the bin function also uses the maximum intensity of the binned spectra, or does it averages it? Also, at the plots below there`s a RT information as a heading fore each plot. Are those plots the BPS of the whole data or the BPC of RT=0.3?]
...
#[Dan: If one want to delve in more details, maybe would be useful to have such values in a table form, for both spectra?]

# Define a function to generate the table for a given spectrum
generate_table <- function(spec) {
  # Extract intensity, retention time, and m/z values
  intensity_values <- intensity(spec)
  mz_values <- mz(spec)
  
  # Filter data based on m/z between 50 and 200
  idx <- which(mz_values >= 50 & mz_values <= 200)
  
  # Subset the values based on the filtered indices
  filtered_intensity <- intensity_values[idx]
  filtered_mz <- mz_values[idx]
  
  # Create a data frame combining these filtered values
  combined_table <- data.frame(
    Intensity = unlist(filtered_intensity),
    MZ = unlist(filtered_mz)
  )
...
# Generate table for spec1
print("Table for spec1:")
print(generate_table(spec1))

# Generate table for spec2
print("Table for spec2:")
print(generate_table(spec2))
...
10 seconds) to the internal standard's values. We perform this matching separate
...
available in our `sampleData`'s `"phenotype"` column.
...
Generally, for LC-MS data, noise can be categorized into three main groups:
...
[phili: should we add references for the above info ([@broadhurst_guidelines_2018])?]
...
can be replaced with random values from the uniform distribution sampling
numbers from half of smallest measured value to the smallest measured value for
a specific feature. The uniform distribution is defined with two parameters
...
We loaded before the *m/z* and expected retention times for the list of 20 IS,
now we need to identify features representing signal from these. Thus, we again
use the `matchValues()` function from the `r Biocpkg("MetaboAnnotation")`
...
contain the *m/z* and retention time values on which to match entities.
...
[jo: original code selected the feature with closest RT, not the single one.]
...
for both raw and normalized data. As anticipated, the CV values for the quality
...
datasets allowing to identify features with excessively high noise. In our data
...
package with the `PercentMissingFilter` setting. Parameter `threshold` of this
filter defines the maximal acceptable percentage of samples with missing
value(s) in at least one of the sample groups defined by parameter `f`. To
consider detected chromatographic peaks per sample, we apply the filter on the
`"raw"` assay of our result object, that contains abundance values only for
detected chromatographic peaks (prior gap-filling). By replacing the `"QC"`
sample group with `NA` in parameter `f` we exclude the QC samples from the
evaluation and consider only the study samples. With `threshold = 40` we keep
only features for which a peak was detected in 2 out of the 3 samples in one of
the sample groups.
...
We therefore evaluate the *dispersion ratio* (Dratio) [@broadhurst_guidelines_2018] for all features in the data set. Using the
...
#' try using using the norm (filled) and see tht would be better. 
...
#' jo: I would also be OK to use in addition the RsdFilter - as long as we
#' identify significant features later and can "annotate" some of them.
```
...
high degree of variability between the QC and study samples. This filtering step
ensures that only features are retained that have considerably lower technical
than biological variance. [jo: reformulated, because constant abundance across
samples could also be interpreted that we remove potentially interesting
samples, that have differences in abundances between the sample group]
...
After the linear models habe been fitted, we can now proceed to extract the
...
control samples. These features are listed, along with their average difference
...
signal. We restrict the MS data set to study samples. Parameter `keepFeatures =
TRUE` ensures that identified features are retained in the subset object.
...
#' Plot the EICs. With parameter `peakBg` we define the (background) color for
#' each individual chromatographic peak in the EIC object.
...
performed at different level of confidence
...
features using functionality provided through Bioconductor packages. Excellent
alternative approaches, and external software tools exist too and will be
discussed later on in the section. [jo: last sentence is maybe a bit clumsy and
needs rephrasing; just don't want to say that we have the best solution here]
...
reproducible R-based analysis workflows. Below we load the resource, list all
available MassBank data sets/releases and load one of them.
...
ions (*[M+Na]+*) or *[M+H-NH3]*) ions. To match the observed *m/z* values
against reference values from these potential ions we use the `matchValues()`
function from the `r Biocpkg("MetaboAnnotation")` package with the
`Mass2MzParam` approach, that allows to specify the types of expected ions with
its `adducts` parameter and the maximal allowed difference between the compared
values using the `tolerance` and `ppm` parameters. Below we first prepare the
data frame with the significant features, set up the parameters for the matching
and perform the comparison of the query features against the reference database
using the `matchValues()` function. With parameter `mzColname` we specify the
name of the column in the query data frame that contains the *m/z* values.
...
see that four of our significant features were matched to the MassBank
...
increase, but requires additional (previous) knowledge and data such as
...
will however represent only noise. The plot below shows the location of
...
compounds). At last we restrict to the data to spectra with at least two
...
jo: thought it might be clearer if we first finish preparing the "query" spectra
before loading the reference data.
...
the `CompareSpectraParam` to define the settings for the matching. With
`requirePrecursor = TRUE` we limit spectra similarity calculations to fragment
spectra with a similar precursor *m/z*. With parameters `tolerance = 0.05` and
`ppm = 40` we define the acceptable difference between compared *m/z*
values. These relaxed tolerance settings ensure that we find matches even if
reference spectra were acquired on instruments with a lower accuracy. With
parameter `THRESHFUN` we define which matches to report. We keep all matches
resulting in a spectra similarity score (calculated with the normalized dot
product [@stein_optimization_1994], the default similarity function) larger than
0.6.
...
As a result we got now all query-target pairs with a spectra similarity higher
than 0.6. Similar to the MS1-based annotation also this result table contains
...
the highest score. As an identifier for a compound (since compound names in
MassBank are not standardized [jo: wrong word; normalized?]) we use its fragment
spectra's InChI-key.
...
pandoc.table(ms2_mtch_res[, c("feature_id", "target_name", "score",
                              "target_inchikey")],
             style = "rmarkdown",
             caption = "MS2 annotation results.",
             split.table = Inf)
```
...
particular disease to reduce the risk of heart arrhythmias.
...
#' [jo: maybe we even keep the QCs in the plot? they would show that even the
#' raw data has high quality, as there is hardly any difference between the
#' signal in QC samples, while there is difference for CTR samples, which is
#' actually to expect -> different individuals]
...
#'place ?
#' [jo: to me it's pretty clear like that. filterRanges makes it much easier
#' /cleaner than matchValues]

#' are you fine with having one match at the end ? It's a nice story i guess

#' [jo: jepp, totally fine. it's a nice story indeed]
```
...
[jo: not sure if we should focus more on the "results", or rather emphasize the
process how we got there, i.e. summarize the technical/methodological aspects of
the workflow; since the topic of this "paper" is the workflow I would maybe
