...
chr_serine <- (chromatogram(data[c(1,4,7,10)], rt = c(175, 186),
...
##' Here again test for multiple ions at once, works but again bc rtime is so different, can see super clearl
...
for (i in 1:length(chr_list)) { 
  ...
  defaultrun <- findChromPeaks(chr_list[[i]], param = cwp )
  print(chr_names)
  print(chromPeaks(defaultrun))
  plot(defaultrun, main = chr_name)
}
