...
plotChromPeakDensity(eic_cystine, param = pdp)
plotChromPeakDensity(eic_Met, param = pdp)
```
...
```{r peaks found in a few QC not all}
...
- compare signal for filled-in and detected signal for QC samples. is it
  correlated?
```{r}
data_p <- fillChromPeaks(data, param = ChromPeakAreaParam(), chunkSize = 5)
...
fts <- rownames(featureValues(data_p))[is.na(rowSums(featureValues(data_p)))]

#' Define m/z - rt regions for these features
mz_rt <- featureArea(data_p, features = fts)

#' Expand the retention time by 1 second on both sides
mz_rt[, "rtmin"] <- mz_rt[, "rtmin"] - 1
mz_rt[, "rtmax"] <- mz_rt[, "rtmax"] + 1

chrs_na <- chromatogram(data_p[1], mz = mz_rt[, c("mzmin", "mzmax")],
